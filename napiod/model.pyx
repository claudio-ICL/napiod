...
STATES = (
    # Down-negative, i.e. mid-price drops and volume imbalance is negative ( <  - .33)
    'd-',
    # Down-neutral, i.e. mid-price drops and volume imbalance is neutral (> - 0.33, < + 0.33)
    'd0',
    # Down-positive, i.e. mid-price drops and volume imbalance is positive (> + 0.33)
    'd+',
    # Stable-negative, i.e. mid-price is unchanged and volume imbalance is negative ( <  - .33)
    's-',
    # Stable-neutral, i.e. mid-price is unchanged and volume imbalance is neutral ( >  - .33, < +.33)
    's0',
    # Stable-positive, i.e. mid-price is unchanged and volume imbalance is positive (< +.33)
    's+',
    # Up-negative, i.e. mid-price increases and volume imbalance is negative ( <  - .33)
    'u-',
    # Up-neutral, i.e. mid-price increases and volume imbalance is neutral ( >  - .33, < +.33)
    'u0',
    # Up-positive, i.e. mid-price increases and volume imbalance is positive (< +.33)
    'u+',
)
...
TOTAL_NUMBER_OF_STATES = NUMBER_OF_POSSIBLE_PRICE_MOVEMENTS * \
    NUMBER_OF_IMBALANCE_SEGMENTS
...
class PriceImpact:
    ...
    def set_orderbook_and_agent(
        ...
        _check_transition_probabilities(
            phis, with_agent=1, agent_direction=self.direction)
